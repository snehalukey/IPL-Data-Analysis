    ...
    cte2 AS (
        SELECT
            *,
            ...
            CASE WHEN runstillfifty >= 100 THEN ballcount ELSE NULL END as ballcountoverhundred,
            CASE WHEN battingteam = team1 THEN team2 ELSE team1 END AS Against
        FROM cte1
    )
        SELECT
            ROW_NUMBER() OVER (PARTITION BY season order by MIN(ballcountoverhundred), SUM(batsman_run) DESC ) AS POS,
            season,
            date AS matdate,
            batter AS Player,
            Against,
            venue,
            MIN(ballcountoverhundred) AS BF,
            SUM(`4s`) AS `4s`,
            SUM(`6s`) AS `6s`,
            SUM(batsman_run) AS Runs
        FROM cte2
        GROUP BY season, date, batter, Against, venue
        HAVING MIN(ballcountoverhundred) IS NOT NULL
